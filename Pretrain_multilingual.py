...
def image_multi_iter(model, image_batch, optimizer, accelerator, metric_logger, device):
    ...
    text_ids_2, text_atts_2, text_ids_masked_2, masked_pos_2, masked_ids_2, image_clip, text_clip = image_batch
    ...
    loss = model(image, text_ids, text_atts, text_ids_masked=text_ids_masked,
                 ...
                 masked_pos_2=masked_pos_2, masked_ids_2=masked_ids_2,image_clip= image_clip, text_clip=text_clip)

    #loss_in_total = loss['loss_itc'] + loss['loss_itm'] + loss['loss_mlm']
    loss_in_total = loss['loss_itc']  + loss['loss_mlm'] + loss['kd_image'] + loss['kd_text']
    # st()
    ...
    metric_logger.update(loss_mm_img_kd_image=loss['kd_image'].item())
    metric_logger.update(loss_mm_img_kd_text=loss['kd_text'].item())
...
def region_multi_iter(model, region_batch, optimizer, accelerator, metric_logger, device):
    ...
    loss_in_total = loss['loss_itc']  + loss['loss_mlm'] + loss['loss_bbox'] + loss['loss_giou']
    accelerator.backward_step(loss_in_total, optimizer)
...
def image_mono_iter(model, mono_batch, optimizer, accelerator, metric_logger, device):
    ...
    # loss_in_total = loss['loss_itc'] + loss['loss_itm'] + loss['loss_mlm']
    loss_in_total = loss['loss_itc'] +  loss['loss_mlm']
    accelerator.backward_step(loss_in_total, optimizer)
...
def train(model, general_loader, region_loader, mono_loader, text_loader, optimizer, epoch_info, device, scheduler, config, accelerator, checkpointer):
    ...
    metric_logger.add_meter('loss_mm_img_kd_image', utils.SmoothedValue(window_size=50, fmt='{value:.2f}'))
    metric_logger.add_meter('loss_mm_img_kd_text', utils.SmoothedValue(window_size=50, fmt='{value:.2f}'))
